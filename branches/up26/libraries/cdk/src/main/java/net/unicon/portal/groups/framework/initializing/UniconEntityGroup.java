 ...
 * 
 */
public class UniconEntityGroup extends GroupMemberImpl implements IEntityGroup
{
 ...
 * UniconEntityGroup
 */
public UniconEntityGroup(String groupKey, Class entityType) 
...
public boolean equals(Object obj)
{
    ...
    if ( obj == this )
        return true;
    if ( ! ( obj instanceof UniconEntityGroup))
        return false;
...
protected java.util.Set primGetAllEntities(Set entities) throws GroupsException
{
    ...
    while ( i.hasNext() )
    {
        ...
        if ( gmi.isEntity() )
            ...
            { ((UniconEntityGroup)gmi).primGetAllEntities(entities); }
    }
    return entities;
}
/**
...
protected java.util.Set primGetAllMembers(Set s) throws GroupsException
{
    ...
    while ( i.hasNext() )
    {
        ...
        s.add(gmi);
        if ( gmi.isGroup() )
            { ((UniconEntityGroup)gmi).primGetAllMembers(s); }
    }
    return s;
}
/**
...
public void setName(java.lang.String newName) throws GroupsException
{
    if ( (getName() != null) && (! getName().equals(newName)) )
    {
        ...
        while ( i.hasNext() )
        {
            UniconEntityGroup eg = (UniconEntityGroup) i.next();
            try
...
{
    return "UniconEntityGroup (" + getKey() + ") "  + getName();
}
/**
...
void initializeMembers(Set groupKeys, Set entityKeys) {
	setMemberEntityKeys(entityKeys);
    setMemberGroupKeys(groupKeys);
    setMemberKeysInitialized(true);
}
