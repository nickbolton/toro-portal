...
public class GuaranteedActionConservative extends GuaranteedActionImpl {
    ...
    public GuaranteedActionConservative(String name, long timeLimit, SimpleBlock action) {
        this(name, timeLimit, action, false);
    }

    public GuaranteedActionConservative(String name, long timeLimit, SimpleBlock action, boolean killActionThread) {
    ...
    public String toString() {
        return "GuaranteedActionConservative " + number;
    }
    ...
    protected void startThreads() {
        ...
        actionSpawnTime = System.currentTimeMillis();
        actionThread.start();
        timeLimitSpawnTime = System.currentTimeMillis();
    ...
    protected UniconThread createTimeLimitThread(long sleepTime) {
        UniconThread answer = new UniconThread("TimeLimit " + number) {
                @Override
                public void run() {
                    timeLimitStartTime = System.currentTimeMillis();
                    guaranteedSleepUntil(actionStartTime + this.sleepTime);
                    if (continueRunning) {
                        timeout();
                    }
                }};
