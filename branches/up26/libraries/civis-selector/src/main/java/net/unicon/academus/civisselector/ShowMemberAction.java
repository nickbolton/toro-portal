...
public final class ShowMemberAction extends CivisSelectorAbstractAction {
	...
	public void init(StateMachine m) {
        ...
        screen = m.getScreen(Handle.create("select_members"));
...
//System.out.println("ShowMemberAction DC " + d_mff.toXml());
			IChoiceCollection c_mff = d_mff.getChoiceCollection();
//System.out.println("ShowMemberAction CC " + c_mff.toXml());
			try{
				IDecision cl = d_mff.getDecision(c_mff
						.getChoice(net.unicon.penelope.Handle
								.create("chooseNameFilter")));
	
				if (cl.getSelections().length > 0) {
					String charSel = cl.getSelections()[0].getOption().getHandle()
							.getValue();
					suc.getSelectorWorkflow().setCharSelection(charSel);
				}
			}catch(Throwable t){
		...
		return new SimpleActionResponse(screen, new SelectMemberQuery(suc));
	}
	...
	private ShowMemberAction(IWarlockFactory owner, Handle handle,
			String[] choices) {
