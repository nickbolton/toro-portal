...
public final class GoUpGroupAction extends CivisSelectorAbstractAction {
    ...
    public static IAction parse(Element e, IWarlockFactory owner)
        ...
        return new GoUpGroupAction(owner, handle, choices);
    ...
    public IActionResponse invoke(IDecisionCollection[] decisions,
        ...
        if(gHistory.hasBack()){            
        	...
        	// if we are at the first group location 
        	if(gHistory.hasGroup()){
        		gHistory.clearLocation();
        	}else{
        		gHistory.goBackPath();
        	}        	
        }
    ...
    private GoUpGroupAction(IWarlockFactory owner, Handle handle,
                                            String[] choices) {
