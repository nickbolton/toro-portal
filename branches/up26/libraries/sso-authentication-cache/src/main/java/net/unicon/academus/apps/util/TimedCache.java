...
package  net.unicon.academus.apps.util;
...
public class TimedCache extends HashMap {
  ...
  public static Map getCache(String cacheName, int iExpirationTimeout) {
      synchronized (caches) {
          Map m = (Map)caches.get(cacheName);
          if (m == null) {
              m = Collections.synchronizedMap(new TimedCache(iExpirationTimeout));
              caches.put(cacheName, m);
          }
  ...
  public static Map getCache(String cacheName) {
      synchronized (caches) {
          Map m = (Map)caches.get(cacheName);
          if (m == null) {
              m = Collections.synchronizedMap(new TimedCache());
              caches.put(cacheName, m);
          }
   ...
   */
  private TimedCache(int iExpirationTimeout) {
    super();
