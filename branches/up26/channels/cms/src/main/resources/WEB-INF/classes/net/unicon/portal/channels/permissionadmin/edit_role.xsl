    ...
    <!--<textarea rows="4" cols="40">
      ...
      <parameter name="editOfferingPermissions"><xsl:value-of select="$editOfferingPermissions" /></parameter>
        ...
        <div class="bounding-box1">
			<form action="{$baseActionURL}?command=update_role&amp;type={$type}" method="post" name="permissionsForm" onsubmit="return validator.applyFormRules(this, new RoleAdminRulesObject());">
				...
				<table cellpadding="0" cellspacing="0" border="0" width="100%">
					<tr>
						...
						<td class="table-content-right" style="text-align:left;" width="100%">
							<input class="text" type="text" name="role_name" size="35" maxlength="35" value="{$role_name}" id="roleAdminTitle">
					...
					<tr>
						...
						<input class="uportal-button" name="submit" value="Update" type="submit" title="Submit permission changes for the '{$role_name}' role"></input>
						<input type="button" class="uportal-button" value="Cancel" onclick="window.locationhref='{$baseActionURL}'" title="Cancel permission changes for the '{$role_name}' role"/></td>
					</tr>
				</table>
