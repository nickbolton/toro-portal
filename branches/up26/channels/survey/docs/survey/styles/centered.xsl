...
<xsl:template match='page' name='form'>
  ...
  <table width='100%' cellpadding='3' cellspacing='0'>
    <xsl:if test='@title'>
      <tr>
        <td align='center' class='uportal-channel-title'><xsl:value-of select='@title'/></td>
      </tr>
      <tr><td><hr/></td></tr>
    </xsl:if>
            ...
            <tr>
              <td valign='top'>
                <table cellpadding='2' cellspacing='0' border='1'>
                  <tr><td align='center' class='uportal-channel-strong'><xsl:value-of select='@question-id'/></td></tr>
                </table>
              </td>
              <td align='center' class='uportal-channel-strong'><xsl:value-of select='text()'/></td>
            </tr>
            <xsl:for-each select='input'>
              <xsl:if test='string-length(normalize-space(text())) > 0'>
                <tr><td colspan='2' align='center' class='uportal-channel-text'><xsl:value-of select='text()'/></td></tr>
              </xsl:if>
...
<!--End: Updating for HREF -->
        ...
        <xsl:otherwise>
          <xsl:choose>
            <xsl:when test='$disabled'><input type='{$type}' name='{$page-id}.{$question-id}.{$input-id}.{$data-ref}' value='{$data-id}' disabled='{$disabled}'/><font class='uportal-channel-text'>
<!--Begin: Updating for HREF -->
				<xsl:choose>
					<xsl:when test="@href"><a href="{@href}" target="new"><xsl:value-of select='text()'/></a></xsl:when>
					<xsl:otherwise><xsl:value-of select='text()'/></xsl:otherwise>
  ...
  <xsl:choose>
    ...
    <xsl:otherwise>
      <table cellpadding='3' cellspacing='0' border='1'>
        <tr><td align='center' class='uportal-channel-text'>&#160;Not Applicable&#160;</td></tr>
      </table>
    </xsl:otherwise>
  </xsl:choose>
</xsl:template>
...
<xsl:template name='statistics'>
  ...
  <td width='45' align='center'>&#160;<font class='uportal-channel-text'><xsl:value-of select='$number-responses'/></font>&#160;</td>
  <td width='45' align='center'>&#160;<font class='uportal-channel-text'><xsl:value-of select='$percent'/></font>&#160;</td>
</xsl:template>
