...
public final class GoToFolderAction extends AbstractWarlockFactory
    ...
    public IActionResponse invoke(IDecisionCollection[] decisions,
        ...
        // Find the target.
        String id = decisions[0].getDecision("chooseFolder").getFirstSelectionHandle();
        ...
        // Change the folder.
        try {
            // check if it a system folder
            if(id.equals(SpecialFolder.SYSFOLDER.getLabel())){
                muc.setFolderSelection(muc.getFactorySelection()
                        .getFactory().getSpecialFolder(SpecialFolder.SYSFOLDER));
            }else{
                muc.setFolderSelection(muc.getFactorySelection().getFactory().getFolder(id));
            }
        } catch (MercuryException me) {
            throw new WarlockException("Unable to switch to requested folder: "+id, me);
        }
    ...
    private GoToFolderAction(IWarlockFactory owner, Handle handle, String[] choices) {
