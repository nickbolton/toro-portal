...
public final class UpdateCredentialsAction extends AbstractWarlockFactory
                                            .AbstractAction {
    ...
    public IActionResponse invoke(IDecisionCollection[] decisions,
        ...
        String username = (String)decisions[0].getDecision("cUsername").getFirstSelectionValue();
        String password = (String)decisions[0].getDecision("cPassword").getFirstSelectionValue();
        String eid = guc.getEntryId();

        SsoEntry[] entries = guc.getAppContext().getSsoEntries();
        if (entries.length == 1 && eid == null)
            eid = entries[0].getHandle();

        guc.getAppContext().getSsoHandler().authenticate(
                guc.getAppContext().getSsoEntry(eid),
                guc.getUserAttributes(),
                username, password);

        guc.setChangeCreds(false);
        guc.clearEntryId();
    ...
    private UpdateCredentialsAction(IWarlockFactory owner, Handle handle, String[] choices) {
...
} // end UpdateCredentialsAction class
