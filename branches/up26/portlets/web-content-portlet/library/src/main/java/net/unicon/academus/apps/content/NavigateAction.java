...
public final class NavigateAction extends AbstractWarlockFactory
                                                .AbstractAction {

    // Instance Members.
    private WebContentApplicationContext app;
    ...
    public void init(StateMachine m) {
        ...
        app = (WebContentApplicationContext) m.getContext();
        screen = m.getScreen(Handle.create("index"));
    ...
    public IActionResponse invoke(IDecisionCollection[] decisions,
        ...
        WebContentUserContext user = (WebContentUserContext) ctx;

        // Choose a page.
        IStateQuery query = null;
        IDecision d = (IDecision) decisions[0].select("@cURL");
        if (d.getSelections().length > 0) {
            // Navigate to a new page.
            ISelection o = (ISelection) d.select("@oURL");
            String p = (String) o.getComplement().getValue();
            ...
            String[] tokens = user.getCurrentPath().split("/");
            int offset = 1 + lvlUp;
            ...
            user.setCurrentPath(path.toString());

        }

        return new SimpleActionResponse(screen, new IndexQuery(app, user));
    ...
    private NavigateAction(IWarlockFactory owner, Handle handle, String[] choices) {
