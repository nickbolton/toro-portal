...
public final class ChangeViewFilterAction extends PermissionsAbstractAction {
	...
	public IActionResponse invoke(IDecisionCollection[] decisions,
	    ...
	    if (decisions.length > 0) {
			...
			if (cl.getSelections().length > 0) {
				String permSel = cl.getSelections()[0].getOption().getHandle()
						.getValue();
				puc.setViewSelection(permSel);
			}
		}
	...
	private ChangeViewFilterAction(IWarlockFactory owner, Handle handle,
			String[] choices) {
