...
public final class NavigateToDestinationAction extends
        AbstractWarlockFactory.AbstractAction {
    ...
    public void init(StateMachine m) {
        ...
        errScreen = m.getScreen(Handle.create("movecopy"));
    ...
    public IActionResponse invoke(IDecisionCollection[] decisions,
        ...
        net.unicon.penelope.Handle h = net.unicon.penelope.Handle
                .create("folderInfoForm");
        for (int i = 0; d_mff == null && i < decisions.length; i++) {
            if (decisions[i].getChoiceCollection().getHandle().equals(
                    h)) {
                d_mff = decisions[i];
        ...
        IChoice navigateChoice = c_mff.getChoice(net.unicon.penelope.Handle
                .create("chooseNavigate"));
        IDecision sf = d_mff.getDecision(navigateChoice);
        ...
        try {
            ...
            String url = sf.getSelections()[0].getOption().getHandle()
                    .getValue();
            ...
            if (url.startsWith("FSA://")) {
                // IFolder...
                try {
                    selectedFolder = AbstractResourceFactory
                            .resourceFromUrl(url);
                    // check if user has view access (useful
                    // in case of a shared factory)
                    AccessRule[] access = user.getAccessRules(
                            selectedFolder.getOwner());
                    if (!FolderQuery.hasAccess(
                            access, BriefcaseAccessType.VIEW)) {
                        throw new RuntimeException("You do not have view " 
                                + "access on the given resource. ");
                    }
                    location = new Location(user.getDriveSelection(),
                            selectedFolder);
                    ...
                    t.printStackTrace();
                    return new SimpleActionResponse(screen, new FolderQuery(
                            new Location(user.getDriveSelection()), app, user,
                            (ErrorMessage[]) err.toArray(new ErrorMessage[err
                                    .size()])));
                }
            } else {
                // Drive...
            ...
            // Adjust the history.
            if (user.getHistory().getLocation().equals(
                    location)) {
                ...
                if (location.before(user.getHistory().getLocation())) {
                    user.getHistory().closeFolder();
                } else {
                    user.getHistory().openFolder();
                }
                user.getHistory().setLocation(
                        location);
            }

        } catch (Throwable de) {
            problem = "The selected folder is not available.";
            solution = "Please select another folder.";
            err.add(new ErrorMessage("other", problem, solution));
            de.printStackTrace();
        }

        return new SimpleActionResponse(errScreen, new SelectDestinationQuery(
                app, user, location, (ErrorMessage[]) err
    ...
    private NavigateToDestinationAction(IWarlockFactory owner, Handle handle,
            String[] choices) {
