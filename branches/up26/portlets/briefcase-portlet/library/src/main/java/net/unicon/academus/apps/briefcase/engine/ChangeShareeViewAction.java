...
public final class ChangeShareeViewAction extends BriefcaseAbstractAction {
    ...
    public static IAction parse(Element e, IWarlockFactory owner)
        ...
        return new ChangeShareeViewAction(owner, handle, choices);
    ...
    public IActionResponse invoke(IDecisionCollection[] decisions,
        ...
        // get the sharee view flag from the decision collection
        IChoiceCollection cCol = decisions[0].getChoiceCollection();
        IDecision d = decisions[0].getDecision(cCol.getChoice(net.unicon.penelope.Handle.create("newAdditionFilter")));
        ISelection s = (ISelection) d.getSelections()[0];
        if(s.getOption().getHandle().getValue().equalsIgnoreCase("new")){
        	((UserEditPreferences)buc.getEditPreferences()).setShowAll(false);
        }else{
        	((UserEditPreferences)buc.getEditPreferences()).setShowAll(true);
        }
    ...
    private ChangeShareeViewAction(IWarlockFactory owner, Handle handle,
                                String[] choices) {
