...
public final class ChangeFolderSortAction extends AbstractWarlockFactory
                                                .AbstractAction {
    ...
    public IActionResponse invoke(IDecisionCollection[] decisions,
        ...
        BriefcaseSortMethod change = null;
        BriefcaseSortMethod current = user.getFolderSortMethod();
        if (current.getMode().equals(mode)) {
            // Toggle the current mode in the other direction.
            change = BriefcaseSortMethod.reverse(current);
        } else {
            // Switch to a different mode.
            change = BriefcaseSortMethod.getInstance(mode + "-asc");
        }
        user.setFolderSortMethod(change);
    ...
    private ChangeFolderSortAction(IWarlockFactory owner, Handle handle,
