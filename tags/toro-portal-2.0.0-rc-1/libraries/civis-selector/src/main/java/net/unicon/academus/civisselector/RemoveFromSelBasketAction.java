...
public final class RemoveFromSelBasketAction extends AbstractWarlockFactory
                                                .AbstractAction {
    ...
    public static IAction parse(Element e, IWarlockFactory owner)
        ...
        return new RemoveFromSelBasketAction(owner, handle, choices, toScreen);
    ...
    public IActionResponse invoke(IDecisionCollection[] decisions,
        ...
        UserGroupHistory gHistory = suc.getSelectorWorkflow().getGroupHistory();

        // Find the target.
        IDecisionCollection dCol = decisions[0];
        
        IChoiceCollection cCol = dCol.getChoiceCollection();
        IDecision d = dCol.getDecision(cCol.getChoice(net.unicon.penelope.Handle.create("selectedItemRemove")));
        String entityId = d.getSelections()[0].getOption().getHandle().getValue();        
        
        suc.getSelectorWorkflow().getSelectionBasket().removeSelection(EntityType.getEntity(entityId));
        ...
        if(query != null){
            ...
            throw new RuntimeException("Remove From Selection Basket was not able to find a screen to go to.");
        }        
    ...
    private RemoveFromSelBasketAction(IWarlockFactory owner, Handle handle,
