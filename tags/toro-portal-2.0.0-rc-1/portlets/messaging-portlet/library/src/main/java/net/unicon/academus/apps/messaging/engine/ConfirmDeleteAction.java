...
public final class ConfirmDeleteAction extends AbstractWarlockFactory
    ...
    public void init(StateMachine m) {
        ...
        screen = m.getScreen(Handle.create("message_delete"));
    ...
    public IActionResponse invoke(IDecisionCollection[] decisions,
        ...
        if (sel.length == 0) {
            msg = "No messages selected for deletion.";
            solution = "Please select the messages you would like to delete, then press the Delete button.";
        } else {
            // the user selected targets...
            try {
                IMessage[] targets = new IMessage[sel.length];

                for (int i=0; i < sel.length; i++) {
                    String msgId = sel[i].getOption().getHandle().getValue();
                    targets[i] = f.getMessage(msgId);
                }

                user.setMessageSelection(targets);
        ...
        if (msg == null) {
            ...
            AttachmentsHelper.cleanupAttachments(user);
        
            rslt = new SimpleActionResponse(screen, new ConfirmDeleteQuery(user));
        } else {
            ErrorMessage[] errors = new ErrorMessage[1];
            errors[0] = new ErrorMessage("other", "Unable to delete messages. "+msg, solution);
