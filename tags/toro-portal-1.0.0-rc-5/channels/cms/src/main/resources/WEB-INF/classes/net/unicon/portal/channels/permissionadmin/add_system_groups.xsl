...
<xsl:include href="common.xsl"/>
    ...
    <div class="bounding-box1">
		<form action="{$baseActionURL}?command=insert_system_group&amp;type={$type}" method="post" name="permissionsForm" onsubmit="return validator.applyFormRules(this, new RoleAdminRulesObject());">
		<input type="hidden" name="targetChannel" value="{$targetChannel}"></input>
		<table cellpadding="0" cellspacing="0" border="0" width="100%">
		<tr>
			<th class="th" colspan="2">Add System Role Group</th>
		</tr>
		<tr>
			...
			<td class="table-content-right" style="text-align:left;" width="100%">
				<input class="text" type="text" name="group_name" size="35" maxlength="35" id="roleAdminTitle"></input><span class="uportal-text-small"> (35 char max)</span>
			</td>
		</tr>
		<tr>
		...
		<tr>
			<td class="table-nav" colspan="2">
				<input class="uportal-button" name="submit" value="Add" type="submit" title="Submit this new system role group"/>
				<input type="button" class="uportal-button" value="Cancel" onclick="window.locationhref='{$baseActionURL}'" title="Cancel adding this new system role group"/>
			</td>
		</tr>
		</table>
		</form>
    ...
    <xsl:template match="activity">
        <li>
            <input name="{../../@handle}-{@handle}" type="checkbox" class="radio" id="roleAdminPermission{label}" style="margin-right: 5px;"></input>
