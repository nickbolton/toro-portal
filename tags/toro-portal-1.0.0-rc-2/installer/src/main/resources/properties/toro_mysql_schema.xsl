...
<statement type="create">
	...
	<xsl:if test="constraints/primary-key">
		<xsl:text>,
CONSTRAINT </xsl:text><xsl:value-of select="constraints/primary-key/name"/><xsl:text> PRIMARY KEY (</xsl:text>
        <xsl:for-each select="constraints/primary-key/columns/column">
            <xsl:value-of select="."/>
