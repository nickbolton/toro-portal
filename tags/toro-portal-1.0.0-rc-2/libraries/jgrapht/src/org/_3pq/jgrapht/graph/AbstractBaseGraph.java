 ...
 * ----------------------
 * (C) Copyright 2003, by Barak Naveh and Contributors.
 * Portions Copyright (C) 2007 Unicon, Inc.
 *
...
public abstract class AbstractBaseGraph extends AbstractGraph implements Graph,
    ...
    public void setAllowMultipleEdges(boolean b) {
        m_allowingMultipleEdges = b;
    }
    ...
    private static class DirectedEdgeContainer implements Serializable {
        List                   m_incoming             = new LinkedList();
        //Map                    m_incomingSources      = new HashMap();
        List                   m_outgoing             = new LinkedList();
        Map                    m_outgoingTargets      = new HashMap();
        ...
        public void addIncomingEdge( Edge e ) {
            m_incoming.add( e );
            //addOrIncrement(m_incomingSources, e.getSource(), e);
        }
        ...
        public void addOutgoingEdge( Edge e ) {
            m_outgoing.add( e );
            addOrIncrement(m_outgoingTargets, e.getTarget(), e);
        }
        ...
        public void removeIncomingEdge( Edge e ) {
            m_incoming.remove( e );
            //removeOrDecrement(m_incomingSources, e.getSource());
        }
        ...
        public void removeOutgoingEdge( Edge e ) {
            m_outgoing.remove( e );
            removeOrDecrement(m_outgoingTargets, e.getTarget());
        }

        public Edge getOutgoingEdge(Object target) {
            Edge rslt = null;
            EdgeCount ec = (EdgeCount)m_outgoingTargets.get(target);
            if (ec != null)
                rslt = ec.getEdge();
            return rslt;
        }

        private void addOrIncrement(Map map, Object obj, Edge edge) {
            EdgeCount ec = (EdgeCount)map.get(obj);
            if (ec == null) {
                ec = new EdgeCount(edge);
                map.put(obj, ec);
            } else {
                ec.increment();
            }
        ...
        private void removeOrDecrement(Map map, Object obj) {
            EdgeCount ec = (EdgeCount)map.get(obj);
            if (ec != null) {
                int i = ec.decrement();
                if (i <= 0)
    ...
    private class DirectedSpecifics extends Specifics implements Serializable {
        ...
        public Edge getEdge( Object sourceVertex, Object targetVertex ) {
            if( containsVertex( sourceVertex )
                ...
                return ec.getOutgoingEdge(targetVertex);
            }
