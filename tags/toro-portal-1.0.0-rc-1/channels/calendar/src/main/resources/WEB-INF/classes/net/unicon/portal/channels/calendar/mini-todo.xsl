...
<!-- If all-calendars we add to personal calendar-->
        ...
        <xsl:variable name="entry" select="/calendar-system/view/entry[todo]" />
        <xsl:variable name="date">
            <xsl:choose>
                <xsl:when test="$entry">
                    <xsl:value-of select="$entry/duration/@start" />
                </xsl:when>
                <xsl:otherwise>
        ...
        <xsl:variable name="hour">
            <xsl:choose>
                <xsl:when test="$entry">
                    <xsl:value-of select="substring-before(substring-after($entry/duration/@start, '_' ), ':' )" />
                </xsl:when>
        ...
        <xsl:variable name="minute">
            <xsl:choose>
                <xsl:when test="$entry">
                    <xsl:value-of select="substring-after(substring-after($entry/duration/@start, '_' ), ':' )" />
                </xsl:when>
...
<!-- Form for add todo -->
        <form method="post" action="{$baseActionURL}">
            ...
            <input type="hidden" name="calid">
                <xsl:attribute name="value">
                    <xsl:value-of select="$calid-add-todo" />
                </xsl:attribute>
            </input>
            <table width="100%" cellpadding="0" cellspacing="0" border="0">
                <tr class="uportal-channel-text">
                    <th class="th-top" colspan="2" align="left">Quick Task Add</th>
                </tr>
                <tr>
                    <td colspan="2" height="3">
                        <img src="{$SPACER}" height="3" alt="" title="" />
                    </td>
                </tr>
                <tr class="uportal-channel-text">
                    <td align="right" width="29">Title</td>
                    <td>
                        <input type="text" name="todo" size="18" class="text">
                            <xsl:attribute name="value">
                                <xsl:value-of select="$entry/todo" />
                            </xsl:attribute>
                        </input>
                    </td>
                </tr>
                <tr class="uportal-channel-text">
                    <td align="right" nowrap="nowrap" width="29">Due</td>
                    <td>
                ...
                <tr class="uportal-channel-text">
                    <td align="right" nowrap="nowrap" width="29">At</td>
                    <td>
                ...
                <tr class="uportal-channel-text">
                    <td width="29" />
                    <td align="left">
                        <input type="hidden" name="default" value="do~addTodo" />
                        <input class="uportal-button" type="submit" name="do~addTodo" value="Add" />
                    </td>
                </tr>
            </table>
        </form>
