...
public final class SubmitBasketAction extends AbstractWarlockFactory
                                                .AbstractAction {
    ...
    public IActionResponse invoke(IDecisionCollection[] decisions,
        ...
        SelectionBasket basket = suc.getSelectorWorkflow().getSelectionBasket();

        IEntity[] entities = basket.getEntities();
        ICivisEntity[] cEntities = new ICivisEntity[entities.length ];
        int cIndex = 0;
        
        // separate out the persons and groups
        for(int i = 0; i < entities.length; i++){            
            cEntities[cIndex++] = (ICivisEntity)entities[i].getObject();            
        }

        // set the person and group list in the user context
        suc.setEntitySelection(cEntities);
        
        // clear the SelectorWorkflow
        suc.setSelectorWorkflow(null);
        
        return new SimpleActionResponse(suc.retrieveScreen(), suc.retrieveQuery());
    ...
    private SubmitBasketAction(IWarlockFactory owner, Handle handle,
                                            String[] choices) {
