...
public final class ConfirmDeleteAction extends AbstractWarlockFactory
                                                .AbstractAction {
    ...
    public void init(StateMachine m) {
        ...
        screen = m.getScreen(Handle.create("delete"));
    ...
    public IActionResponse invoke(IDecisionCollection[] decisions,
        ...
        IDecision d = dCol.getDecision(cCol.getChoice(net.unicon.penelope.Handle.create("selectedItems")));
        ...
        try{
            ...
            msg = "Resource with the given name was not found. ";
            problem = "The given resource may have been remamed or deleted. " +
                        "'. " + msg;
            err.add(new ErrorMessage("other", problem, ""));
            t.printStackTrace();
            return new SimpleActionResponse(screen, new FolderQuery(user.getHistory().getLocation()
                    , app, user
        ...
        while (it.hasNext()) {
            ...
            if (r.getOwner().getRoot().equals(r)) {
                problem = "Unable to delete folder '"
                                    + r.getName()
        ...
        // Check to see if any of the selected folders are shared roots.
/* ToDo:  Restore this functionality...
        user.updateSharedResources();
        List sharedResources = user.getSharedResources();
        for (int i = 0; i < targets.length; i++) {
            if (sharedResources.contains(targets[i].getUrl())) {
                problem = "Unable to delete folder '" + targets[i].getName()
                        + "'.  Shared folder roots may not be"
                        + "moved, copied, or deleted.";
                solution = "Please remove any sharing from this folder"
                        + " before deleting.";
                err.add(new ErrorMessage("other", problem, solution));
            }
        }
*/
        ...
        List access = Arrays.asList(user.getAccessRules(targets[0].getOwner()));
        ...
        if (!access.contains(new AccessRule(BriefcaseAccessType.DELETE, true))) {
            ...
            problem = "Unable to delete '" + targets[0].getName() + "' folder.  "
                        + msg;
            err.add(new ErrorMessage("other", problem, solution));
            hasPermission = false;
        }
        ...
        if (err.size() == 0) {
            ...
            return new SimpleActionResponse(screen, new ConfirmDeleteQuery(targets));
        } else {
            // Display the error.
    ...
    private ConfirmDeleteAction(IWarlockFactory owner, Handle handle, String[] choices) {
