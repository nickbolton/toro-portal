...
public final class ChangeRecipientSortAction extends AbstractWarlockFactory
                                                .AbstractAction {
    ...
    public IActionResponse invoke(IDecisionCollection[] decisions,
        ...
        RecipientSortMethod change = null;
        RecipientSortMethod current = muc.getRecipientSortMethod();
        
        if (current.getMode().equals(mode)) {
            // Toggle the current mode in the other direction.
            change = RecipientSortMethod.reverse(current);
        } else {
            // Switch to a different mode.
            change = RecipientSortMethod.getInstance(mode + "-asc");
        }
        muc.setRecipientSortMethod(change);
    ...
    private ChangeRecipientSortAction(IWarlockFactory owner, Handle handle,
