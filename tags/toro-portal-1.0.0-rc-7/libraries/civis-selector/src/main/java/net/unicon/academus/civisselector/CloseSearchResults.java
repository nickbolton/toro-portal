...
public final class CloseSearchResults extends AbstractWarlockFactory
                                                .AbstractAction {

    // Instance Members.
    private IScreen groupScreen;
    private IScreen memberScreen;
    private IScreen basketScreen;
    ...
    public void init(StateMachine m) {
        ...
        basketScreen = m.getScreen(Handle.create("select_basket"));
    ...
    public IActionResponse invoke(IDecisionCollection[] decisions,
        ...
        String toScreen = workflow.getLastScreen();
        IScreen screen = this.groupScreen;
        IStateQuery query = new SelectGroupQuery(suc);

        if(toScreen.equals("select_basket")){
            query = new SelectBasketQuery(suc);
            screen = this.basketScreen;
        }
    ...
    private CloseSearchResults(IWarlockFactory owner, Handle handle,
                                            String[] choices) {

        super(owner, handle, choices);

        // Instance Members.
        this.basketScreen = null;
