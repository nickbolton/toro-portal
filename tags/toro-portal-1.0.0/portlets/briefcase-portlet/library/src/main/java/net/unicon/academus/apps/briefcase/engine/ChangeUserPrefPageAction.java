...
public final class ChangeUserPrefPageAction extends BriefcaseAbstractAction {
    ...
    public IActionResponse invoke(IDecisionCollection[] decisions,
        ...
        // if paging in this page, need to save the users permission choices in memory 
        ps = buc.getEditPreferences().getPageState();
        if(decisions.length < 1){
            String msg = "Argument 'decisions' cannot be empty.";
            throw new IllegalArgumentException(msg);
        }
        updatePermissions(decisions[0], buc);
        buc.setResourceSelection(new IFolder[] {(IFolder) buc.getHistory().getLocation().getObject()});
