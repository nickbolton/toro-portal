...
public final class ShowFolderAction extends AbstractWarlockFactory
                                            .AbstractAction {
    ...
    public IActionResponse invoke(IDecisionCollection[] decisions,
        ...
        // Get the last folderview location (fromLocation if move/copy)
        Location location = 
            (Location) user.getSession().getAttribute("fromLocation");
        if (location == null) {
            location = user.getHistory().getLocation();
        } 

        // Clear the selections (if any).
        user.clearResourceSelection();
        
        //  Purge any session attributes related to move/copy (if any)
        user.getSession().removeAttribute("fromLocation");
        user.getSession().removeAttribute("destinationMode");

        // Clear out the sharing selection basket if necessary.
        if(user.hasTargetSelection()){
            user.getTargetSelection().clearShareeSelection();
        }

        // Reset all sharee paging to first page.
        user.clearSharingPageSelections();

        return new SimpleActionResponse(screen, new FolderQuery(location, app, user));
    ...
    private ShowFolderAction(IWarlockFactory owner, Handle handle, String[] choices) {
