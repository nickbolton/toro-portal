...
public final class UploadAction extends AbstractWarlockFactory
    ...
    public IActionResponse invoke(IDecisionCollection[] decisions,
        ...
        // Add each item.
        ISelection[] s = decisions[0].getDecision("fileUploads").getSelections();
        for (int i = 0; uploaded && i < s.length; i++) {
            FileUpload fu = (FileUpload)s[i].getComplement().getValue();
            ...
            if (maxSize != 0 && fu.getSize() > maxSize) {
                ...
                solution = "Please double-check the file size of the uploaded file"
                    ...
                    + "Zipping, or otherwise compressing, the file may result "
                    + "in smaller file sizes.";
                uploaded = false;
            } else {
                try {
                    draft.addAttachment(fu.getName(), fu.getContentType(), fu.getContents());
                } catch (Throwable t) {
                    msg = t.getMessage();
                    uploaded = false;
                }
        ...
        if (uploaded) {
            return new SimpleActionResponse(screen, new ComposeMessageQuery(muc));
        } else {
            ...
            return new SimpleActionResponse(screen, new ComposeMessageQuery(muc, errors));
        }
