...
public final class ReturnViewAction extends AbstractWarlockFactory
                                            .AbstractAction {
    ...
    public IActionResponse invoke(IDecisionCollection[] decisions,
        ...
        // Clean up any prepared exports
        AttachmentsHelper.cleanupExport(muc);
        ...
        if (muc.getDetailPref().getCurrentMessageIndex() != -1) {
            ...
            AttachmentsHelper.prepareAttachments(muc,
                    muc.getMessageList()[muc.getDetailPref().getCurrentMessageIndex()]);

            // Go back to detail view
            rslt = new SimpleActionResponse(screenDetail, new ReadMessageQuery(muc));
        } else {
            // Go to message list
            rslt = new SimpleActionResponse(screenList, new MessageListQuery(muc));
        }
        return rslt;
    }
    ...
    private ReturnViewAction(IWarlockFactory owner, Handle handle, String[] choices) {
