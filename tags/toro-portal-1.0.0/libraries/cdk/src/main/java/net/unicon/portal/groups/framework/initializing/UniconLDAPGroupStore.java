...
/**
 * UniconLDAPGroupStore.
 * @author Nick Bolton
 */
public class UniconLDAPGroupStore implements IEntityGroupStore, IEntityStore, IEntitySearcher {
  ...
  public UniconLDAPGroupStore() {
    Document config = null;
    try{
      config = ResourceLoader.getResourceAsDocument(this.getClass(),"/properties/groups/LDAPGroupStoreConfig.xml");
    }
    catch(Exception rme){
      throw new RuntimeException("UniconLDAPGroupStore: Unable to find configuration configuration document");
    }
    init(config);
  }

  public UniconLDAPGroupStore(Document config){
    init(config);
  }
  ...
  protected void init(Document config){
    ...
    if (nl.getLength() == 1){
      ...
      throw new RuntimeException("UniconLDAPGroupStore: config file must contain one config element");
    }
      ...
      switch (type){
        case FILTER:
          ...
          try {
            ...
            LogService.log(LogService.ERROR,"UniconLDAPGroupStore: Unable to perform filter "+filter);
        ...
        case ATTRIBUTES:
          ...
          try {
            ...
            LogService.log(LogService.ERROR,"UniconLDAPGroupStore: Unable to perform attribute search");
            LogService.log(LogService.ERROR,nex);
          }
  ...
  protected void processLdapResults(NamingEnumeration results, ArrayList keys){
    ...
    try{
      while(results.hasMore()){
        ...
        LogService.log(LogService.ERROR,"UniconLDAPGroupStore: error processing results");
       LogService.log(LogService.ERROR,nex);
    }
    finally{
  ...
  protected DirContext getConnection(){
    ...
    if (context==null){
      ...
      try {
        ...
        LogService.log(LogService.ERROR,"UniconLDAPGroupStore: unable to get context");
        LogService.log(LogService.ERROR,nex);
      }
  ...
  protected IEntityGroup makeGroup(GroupShadow shadow) throws GroupsException{
    IEntityGroup group = new UniconEntityGroup(shadow.key,iperson);
  ...
  public void delete(IEntityGroup group) throws GroupsException {
    throw new java.lang.UnsupportedOperationException("UniconLDAPGroupStore: Method delete() not supported.");
  }
  ...
  public IEntityGroup newInstance(Class entityType) throws GroupsException {
    throw new java.lang.UnsupportedOperationException("UniconLDAPGroupStore: Method newInstance() not supported");
  }
  public void update(IEntityGroup group) throws GroupsException {
    throw new java.lang.UnsupportedOperationException("UniconLDAPGroupStore: Method update() not supported");
  }
  public void updateMembers(IEntityGroup group) throws GroupsException {
    throw new java.lang.UnsupportedOperationException("UniconLDAPGroupStore: Method updateMembers() not supported");
  }
  public ILockableEntityGroup findLockable(String key) throws GroupsException {
    throw new java.lang.UnsupportedOperationException("UniconLDAPGroupStore: Method findLockable() not supported");
  }
    ...
    switch (method){
      case IS:
        for (i=0; i<g.length;i++){
          if(g[i].name.toUpperCase().equals(query.toUpperCase())){
  ...
  throws GroupsException {
    ...
    try {
      ...
      LogService.log(LogService.ERROR,"UniconLDAPGroupStore: Unable to perform filter "+query);
      LogService.log(LogService.ERROR,nex);
    }
    ArrayList keys = new ArrayList();
    processLdapResults(userlist,keys);
