...
<xsl:template match="/">
  <xsl:call-template name="links">
  	<xsl:with-param name="curPage">preferences</xsl:with-param>
  </xsl:call-template>
  
  <h2 class="page-title">Edit Preferences</h2>
  ...
  <div class="bounding-box1">
	  <form name="editPreferencesForm" action="{$baseActionURL}" method="post">
		...
		<table cellpadding="0" cellspacing="0" border="0" width="100%">
		  ...
		  <tr>
			<td class="table-light-left" style="vertical-align:top;" nowrap="nowrap"><label for="cacep1">Page Size</label></td>
			<td class="table-content-right">
				<select name="selectPageSize" id="cacep1">
		  ...
		  <tr>
			<td class="table-light-left" style="vertical-align:top;" nowrap="nowrap"><label for="cacep2">Maximum Announcement<br />Age (In Days)</label></td>
			<td class="table-content-right">
				<select name="selectMaximumAge" id="cacep2">
		  ...
		  <tr>
			<td colspan="2" class="table-nav">
				<nobr>
				<input type="submit" class="uportal-button" name="Submit" value="Submit" title="Click to submit preference edits and return to viewing all announcements"/>
				<input type="submit" class="uportal-button" name="Submit" value="Cancel" title="Cancel editing preferences and return to viewing all announcements"/>
				</nobr>
			</td>
		  </tr>
		</table>
...
<xsl:template match="category">
  ...
  <xsl:variable name="VALUE"><xsl:value-of select="@value"/></xsl:variable>
	...
	<xsl:if test="$SELECTED = 'true'">
		<li><input name="selectedCategories" value="{$VALUE}" type="checkbox" class="radio" checked="checked" /><xsl:value-of select="." /></li>
	</xsl:if>
	<xsl:if test="$SELECTED = 'false'">
		<li><input name="selectedCategories" value="{$VALUE}" type="checkbox" class="radio" /><xsl:value-of select="." /></li>
	</xsl:if>
</xsl:template>
