...
public final class ToggleNameSorting extends CivisSelectorAbstractAction {

    // Instance Members.    
    private String toScreen;
    private IScreen screen;
    private IApplicationContext bac;
    ...
    public IActionResponse invoke(IDecisionCollection[] decisions,
        ...
        if(toScreen.equals("select_groups")){
            suc.getSelectorWorkflow().getGroupPreferences().toggleSort();	        
	        query = new SelectGroupQuery(suc);
        }    
        if(toScreen.equals("select_basket")){
            suc.getSelectorWorkflow().getSelBasketPreferences().toggleSort();
            query = new SelectBasketQuery(suc);
        }
        if(toScreen.equals("select_members")){
            suc.getSelectorWorkflow().getMemberPreferences().toggleSort();
            query = new SelectMemberQuery(suc);
        }       
        if(toScreen.equals("select_search_results")){
            suc.getSelectorWorkflow().getSearchShareePreferences().toggleSort();
    ...
    private ToggleNameSorting(IWarlockFactory owner, Handle handle,
