...
public class SetShowMemberAction extends CivisSelectorAbstractAction{
    ...
    public IActionResponse invoke(IDecisionCollection[] decisions,
        ...
        IDecision d = dCol.getDecision(cCol.getChoice(net.unicon.penelope.Handle.create("chooseMemberFilter")));
        
        String shareMember = d.getSelections()[0].getOption().getHandle().getValue();
        if(shareMember.equals("hide")){           
            suc.getSelectorWorkflow().getGroupHistory().setShowMembers(false);
        }else{
            suc.getSelectorWorkflow().getGroupHistory().setShowMembers(true);
        }
    ...
    private SetShowMemberAction(IWarlockFactory owner, Handle handle,
                                            String[] choices) {
