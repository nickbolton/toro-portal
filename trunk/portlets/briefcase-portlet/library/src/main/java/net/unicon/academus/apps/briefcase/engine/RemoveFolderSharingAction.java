...
public final class RemoveFolderSharingAction extends BriefcaseAbstractAction {
    ...
    public void init(StateMachine m) {
        ...
        if (m == null) {
         ...
         screen = m.getScreen(Handle.create("addedit_folder"));
    ...
    public IActionResponse invoke(IDecisionCollection[] decisions,
        ...
        // get the folder whose sharing is to be removed.
        IDecisionCollection dc = decisions[0];
        IChoiceCollection cc = dc.getChoiceCollection();
        IDecision dFolderId = dc.getDecision(cc.getChoice(net.unicon.penelope.Handle.create("cId")));
        ISelection sFolderId = dFolderId.getSelections()[0];

        EncryptionService encryptionService = bac.getEncryptionService();
        String url = sFolderId.getOption().getHandle().getValue();
        url = encryptionService.decrypt(url);
        IFolder f = null;
        try {
            f = (IFolder)AbstractResourceFactory.resourceFromUrl(url);
        
	        // get the entries for the given folder
	        Drive sharedDrive = bac
	        .getDrive(buc.getDriveSelection().getShareTarget());
	        IAccessEntry[] entries = 
	        	bac.getSharedEntries(f, buc.getUsername(), sharedDrive);  
	        
	        AccessBroker broker = sharedDrive.getBroker();
            ...
            String problem = "The selected folder is no longer accessible. ";
            err[0] = new ErrorMessage("other", problem, "");
            e.printStackTrace();
            return new SimpleActionResponse(screen, new FolderQuery(buc.getHistory().getLocation(), bac,
                buc, err));
        }
 
        return new SimpleActionResponse(screen, new EditFolderQuery(f, false, true, bac, buc));
    ...
    private RemoveFolderSharingAction(IWarlockFactory owner, Handle handle,
                                            String[] choices) {
