...
public class UserAscComparator implements Comparator {
	...
	public int compare(Object user1, Object user2) {
		int rslt = 0;
		try {
			rslt = ((IAcademusUser)user1).getLastName().toLowerCase()
					.compareTo(((IAcademusUser)user2).getLastName()
					.toLowerCase());
			if (rslt == 0) {
				rslt = ((IAcademusUser)user1).getFirstName().toLowerCase()
				.compareTo(((IAcademusUser)user2).getFirstName()
				.toLowerCase());
			}
		} catch (AcademusFacadeException e) {
			throw new RuntimeException("UserAscComparator: Unable to get " +
					"user's last or first name.", e);
		} 
		return rslt;
	}
}
